...
이것을 테스트하기 실행코드 샘플이 포함되어 있습니다.

### 샘플 실행하는 법
./executable/Animator.exe를 실행합니다.

또는

Renderer.sln에서 Animator를 시작 프로젝트로 설정하고 F5를 눌러 디버깅모드로 실행합니다.
...
## 왜 만들었는가
이야기는 2011년, 김상병이었던 시절로 거슬러 갑니다. 건축학도로서 맨날 써 온 캐드가 어떻게 작동하는지, 열심히 하던 게임들은 어떻게 만드는지 문득 궁금해서 프로그래밍 공부를 시작했습니다. 그때 화면에 점 찍는 법만 누가 알려준다면 3차원을 구현해 볼 수 있을 것 같다는 생각을 했었는데, 군인으로서는 얻을 수 있는 자료가 너무 한정적이어서 프로그래밍의 맛만 보고 끝났습니다.

그로부터 9년 후, 우여곡절 끝에 개발자가 되기로 했습니다. 목표 진로는 막연하게 캐드, 게임, 그래픽, 인공지능, 로봇 등 이었습니다.

그 후로 1년 반 정도 기초를 탄탄하게 다졌고, 드디어 OpenGL을 공부했습니다.
